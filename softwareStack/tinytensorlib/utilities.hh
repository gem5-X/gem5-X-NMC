...
printLayerInfo(conv_layer_args * conv_args)
{
    ...
    cout << "Stride: " << conv_args->stride << "\t|"; 
    cout << "Padding: " << conv_args->padding; 
    cout << endl;
    return;
}
...
printVector(TB_Vector v, int limit=-1)
{
    ...
    for (int i = 0; i < l; i++) {
        cout <<  static_cast<Eigen::half>(v(i)) << "\t";
    } cout << endl;
...
{
    int lx = (limit_x < 0) ? m.dimensions()[1] : limit_x;
    ...
    for (int i = 0; i < ly; i++) {
        for (int j = 0; j < lx; j++) {
            cout << static_cast<Eigen::half>(m(i, j)) << "\t";
        } cout << endl;
    } cout << endl;
}
...
{
    int lx = (limit_x < 0) ? m.dimensions()[2] : limit_x;
    int ly = (limit_y < 0) ? m.dimensions()[1] : limit_y;

    cout << "Matrix contents, channel " << channel << ":\n";
    for (int i = 0; i < ly; i++) {
        for (int j = 0; j < lx; j++) {
            cout << static_cast<Eigen::half>(m(channel, i, j)) << "\t";
        } cout << endl;
    } cout << endl;
}
